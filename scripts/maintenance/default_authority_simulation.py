...
def _print_report(report: dict[str, Any]) -> None:
    ...
    print(
        "status={status} actual_default={actual} simulated_default={simulated} authority_switch_performed={switched}".format(
            status=report.get("status"),
            actual=sim.get("actual_default"),
            simulated=sim.get("simulated_default"),
            switched=sim.get("authority_switch_performed"),
        )
    )
    print(
        "  simulation_only=True trace_emission_preserved={trace} rollback_supported={rollback}".format(
            trace=sim.get("trace_emission_preserved"),
            rollback=sim.get("rollback_supported"),
        )
    )
    ...
    print(
        "  rollback_supported={supported} method={method}".format(
    ...
    if commands:
        print("  rollback_commands=" + " | ".join(commands))


def main() -> int:
    parser = argparse.ArgumentParser(description="Default authority simulation (observational only)")
    parser.add_argument("run_or_path", nargs="?", help="Run ID or run directory path")
    parser.add_argument("--latest", action="store_true", help="Simulate latest run")
    parser.add_argument("--all", action="store_true", help="Simulate all runs")
    ...
    try:
        if args.summary or args.all:
            payload = evaluate_default_authority_simulation(None, runs_root=runs_root, recent=args.recent)
        ...
        report = evaluate_default_authority_simulation(target, runs_root=runs_root)
