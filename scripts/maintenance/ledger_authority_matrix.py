...
def _print_report(report: dict[str, Any]) -> None:
    ...
    print(
        'status={status} cutover_ready={cutover} recommended_mode={mode}'.format(
            status=report.get('status'),
            cutover=decision.get('cutover_ready'),
    ...
    rollback = report.get('matrix', {}).get('rollback', {})
    if rollback:
...
def main() -> int:
    parser = argparse.ArgumentParser(description='Ledger authority readiness matrix')
    ...
    try:
        if args.summary or args.all:
            payload = evaluate_ledger_authority_readiness(None, runs_root=runs_root, recent=args.recent)
        ...
        report = evaluate_ledger_authority_readiness(target, runs_root=runs_root)
